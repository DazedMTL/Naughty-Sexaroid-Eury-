...
//Array型、各要素はobject
const L2DINmodels = JSON.parse(
  JSON.stringify(L2DINPP["Modelcondition"], L2DINstringifyReplacer)
);
...
Game_Live2d.prototype.live2dupdateMove = function (model_no) {
  ...
  if (d > 0) {
    $gameLive2d.pos_x[model_no] =
      ($gameLive2d.pos_x[model_no] * (d - 1) + $gameLive2d._targetX[model_no]) /
      d;
    $gameLive2d.pos_y[model_no] =
      ($gameLive2d.pos_y[model_no] * (d - 1) + $gameLive2d._targetY[model_no]) /
      d;
...
Live2DSprite.prototype.createShader = function () {
  // バーテックスシェーダーのコンパイル
  var vertexShaderId = $gameLive2d.gl.createShader(
    $gameLive2d.gl.VERTEX_SHADER
  );
  ...
  // フラグメントシェーダのコンパイル
  var fragmentShaderId = $gameLive2d.gl.createShader(
    $gameLive2d.gl.FRAGMENT_SHADER
  );
...
Live2DSprite.prototype._renderWebGL = function (renderer) {
  ...
  if (!this.frameBuffer) {
    this.frameBuffer = $gameLive2d.gl.getParameter(
      $gameLive2d.gl.FRAMEBUFFER_BINDING
    );
  }
  // 透過設定
  ...
  if (!this.modelReady) {
    ...
    this.texture = PIXI.RenderTexture.create(
      $gameLive2d.canvas.width,
      $gameLive2d.canvas.height
    );
    this.createShader();
  ...
  const elementArrayBuffer = temp_gl.getParameter(
    temp_gl.ELEMENT_ARRAY_BUFFER_BINDING
  );
  ...
  //Slip 2017/03/24
  var vertexAttr0Enabled,
    vertexAttr1Enabled,
    vertexAttr2Enabled,
    vertexAttr3Enabled;

  if (!useVAO) {
    vertexAttr0Enabled = temp_gl.getVertexAttrib(
      0,
      temp_gl.VERTEX_ATTRIB_ARRAY_ENABLED
    );
    vertexAttr1Enabled = temp_gl.getVertexAttrib(
      1,
      temp_gl.VERTEX_ATTRIB_ARRAY_ENABLED
    );
    vertexAttr2Enabled = temp_gl.getVertexAttrib(
      2,
      temp_gl.VERTEX_ATTRIB_ARRAY_ENABLED
    );
    vertexAttr3Enabled = temp_gl.getVertexAttrib(
      3,
      temp_gl.VERTEX_ATTRIB_ARRAY_ENABLED
    );
  }
  ...
  const _activeTextureLocation = renderer._activeTexture
  ...
  if (!useVAO) {
    vertexAttr0Enabled
      ? temp_gl.enableVertexAttribArray(0)
      : temp_gl.disableVertexAttribArray(0);
    vertexAttr1Enabled
      ? temp_gl.enableVertexAttribArray(1)
      : temp_gl.disableVertexAttribArray(1);
    vertexAttr2Enabled
      ? temp_gl.enableVertexAttribArray(2)
      : temp_gl.disableVertexAttribArray(2);
    vertexAttr3Enabled
      ? temp_gl.enableVertexAttribArray(3)
      : temp_gl.disableVertexAttribArray(3);
  }
  scissorTestEnabled
    ? temp_gl.enable(temp_gl.SCISSOR_TEST)
    : temp_gl.disable(temp_gl.SCISSOR_TEST);
  stencilTestEnabled
    ? temp_gl.enable(temp_gl.STENCIL_TEST)
    : temp_gl.disable(temp_gl.STENCIL_TEST);
  depthTestEnabled
    ? temp_gl.enable(temp_gl.DEPTH_TEST)
    : temp_gl.disable(temp_gl.DEPTH_TEST);
  cullFaceEnabled
    ? temp_gl.enable(temp_gl.CULL_FACE)
    : temp_gl.disable(temp_gl.CULL_FACE);
...
(function () {
  ...
  const Game_Interpreter_updateWaitCount =
    ...
    if (command === StrForComparison) {
      ...
      switch (args[1]) {
        ...
        case "モーション":
          ...
          Live2DManager.prototype.live2dMotion(
            model_no,
            args[2],
            args[3],
            loop
        ...
        case "left":
        case "左":
          Live2DManager.prototype.live2dSetPosition_X(
            model_no,
            $gameLive2d._pos_left
          );
        ...
        default:
          ...
          Live2DManager.prototype.live2dSequenceMotion(
            model_no,
            innerMotionName,
            loop
          );
...
Live2DManager.prototype.live2dMotion = function (
  ...
  $gameLive2d._lappLive2dManager._models
    .at(model_no - 1)
    .changeMotion(motionGroup, motion_no - 1, loop);
...
Live2DManager.prototype.live2dSequenceMotion = function (
  model_no,
  motions,
  loop
) {
      ...
      if ($gameParty.members()[i]._name == $gameLive2d._name[number]) {
        for (var j = 0; j < 5; j++) {
          ...
          if ($gameLive2d.linkEquip_None[number][j] != "") {
            ...
            innerMotionName = $gameLive2d.InnerMotionName(
              $gameLive2d._name[number],
              cloth_name
            );
            var modelNo = Live2DManager.prototype.getNumberFromName(
              $gameLive2d._name[number]
            );

            $gameLive2d._lappLive2dManager._models
              .at(modelNo - 1)
              .setClothMotion(innerMotionName);
          }
        }
      }
    }
...
if (L2DINuseinbattle) {
  ...
  const Battle_L2DINcreateWindowLayer =
