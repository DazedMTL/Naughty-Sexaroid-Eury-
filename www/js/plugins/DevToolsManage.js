...
(function () {
  ...
  const paramStartupDevTool = getParamBoolean([
    "StartupDevTool",
    "開始時に起動",
  ]);
  const paramFuncKeyReload = getParamString(["FuncKeyReload", "リロードキー"]);
  const paramFuncKeyOnTop = getParamString([
    "FuncKeyOnTop",
    "最前面に表示キー",
  ]);
  const paramFuncKeyRapidGame = getParamString([
    "FuncKeyRapidGame",
    "高速化切替キー",
  ]);
  const paramFuncKeyVictory = getParamString([
    "FuncKeyForceVictory",
    "強制戦闘勝利キー",
  ]);
  const paramFuncKeyDefeat = getParamString([
    "FuncKeyForceDefeat",
    "強制戦闘敗北キー",
  ]);
  const paramFuncKeyAbort = getParamString([
    "FuncKeyForceAbort",
    "強制戦闘中断キー",
  ]);
  const paramFuncKeyScript = getParamString([
    "FuncKeyScript",
    "スクリプト実行キー",
  ]);
  ...
  const paramShiftRightOnBlur = getParamNumber(
    ["ShiftRightOnBlur", "右寄せ座標"],
    -1000,
    1000
  );
  ...
  const paramInvalidMessageSkip = getParamBoolean([
    "InvalidMessageSkip",
    "メッセージスキップ無効",
  ]);
  ...
  SceneManager.initDevCommand = function () {
    this.devCommands = [
      ...
      },
      {
        ...
        use: true,
        name: "画面フリーズ",
        key: paramFuncKeyFreeze,
        type: "checkbox",
      },
      {
        code: 108,
        use: !!SceneManager.takeCapture,
        name: "画面キャプチャ",
        key: true,
        type: "normal",
      },
      ...
      },
      {
        ...
        use: true,
        name: "戦闘強制中断",
        key: paramFuncKeyAbort,
        type: "normal",
      },
      {
        code: 111,
        use: !!SceneManager.onKeyDownForScreenMovie,
        name: "画面録画",
        key: true,
        type: "normal",
      },
  ...
  SceneManager.onKeyDownForDevToolManage = function (event) {
    ...
    if (commandCode && this._nwJsGui)
      this.executeDevCommand(commandCode, event);
  };
  ...
  SceneManager.updateDocumentTitle = function () {
    ...
    document.title =
      this.originalTitle +
      this.addDocumentTitleRapidOrSlow() +
  ...
  SceneManager.showScriptDialog = function () {
    ...
    const scriptString = window.prompt(
      promptValue,
      this._nwJsGui.readClipboard()
    );
  ...
  StorageManager.loadFromLocalFileJson = function (savefileId) {
    const fs = require("fs");
    const filePath =
      this.localFileDirectoryPath() + "file%1.json".format(savefileId);
  ...
  Scene_Base.prototype.fadeSpeed = function () {
    return SceneManager.isRapid()
      ? 1
      : _Scene_Base_fadeSpeed.apply(this, arguments);
  };
  ...
  Scene_Map.prototype.isFastForward = function () {
    return (
      _Scene_Map_isFastForward.apply(this, arguments) || SceneManager.isRapid()
    );
  };
  ...
  Scene_Title.prototype.createGreetingSprite = function () {
    ...
    if (message) {
      ...
      this._greetingSprite.bitmap.drawText(
        message,
        0,
        0,
        1000,
  ...
  Scene_Title.prototype.update = function () {
    ...
    if (this._greetingSprite) {
      ...
      this._greetingSprite.y =
        Graphics.height / 2 + Math.sin(Graphics.frameCount / 30) * 30;
  ...
  Game_Event.prototype.getInformation = function () {
    return (
      `ID:${this._eventId} Name:${this.event().name}` +
      Game_Character.prototype.getInformation.call(this)
    );
  };
  ...
  Controller_NwJs.prototype.makeMenuItem = function (commandInfo, menuObject) {
    ...
    const key =
      commandInfo.key && commandInfo.key[0] === "F" ? commandInfo.key : "";
    ...
    menuItem.click = function () {
      ...
      if (commandInfo.type === "checkbox")
        this._clickMenuItems[commandInfo.name].checked = result;
    }.bind(this);
  ...
  Controller_NwJs.prototype.getSortedDevCommands = function () {
    return SceneManager.devCommands
      .filter((command) => command.key && command.key !== "none")
      .sort(function (a, b) {
        if (a.key && b.key) {
          return (
            Input.functionReverseMapper[a.key] -
            Input.functionReverseMapper[b.key]
          );
        } else if (a.key || b.key) {
          return a.key ? -1 : 1;
        } else {
  ...
  Controller_NwJs.prototype.prompt = function (
    value,
    defaultValue,
    aliasPrompt
  ) {
