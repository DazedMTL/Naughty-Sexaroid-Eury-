...
const L2DPLE_DefaultBodyPicture = JSON.parse(
  JSON.stringify(L2DPLE["DefaultBodyPicture"], L2DINstringifyReplacer)
);
      ...
      if ($gameParty.members()[i]._name == $gameLive2d._name[number]) {
        for (var j = 0; j < 5; j++) {
          ...
          if ($gameLive2d.linkEquip_None[number][j] != "") {
            ...
            //ピクチャへの反映
            $gameLive2d_linkEquipment.setPicture(
              i,
              j,
              cloth_name,
              $gameLive2d.linkEquip_None[number][j]
            );
...
Live2DPictureLinkEquipment.prototype.ApplyClothdataFromParty = function () {
  for (var i = 0; i < $gameParty._actors.length; i++) {
    ...
    for (var j = 0; j < L2DPLE_DefaultBodyPicture.length; j++) {
      if (
        $gameParty.members()[i]._name == L2DPLE_DefaultBodyPicture[j].modelName
      ) {
        pictureName = this.getPictureName(
          L2DPLE_DefaultBodyPicture[j].modelName
        );
        break;
      } else {
        pictureName = "";
    ...
    this._body[i] = new PictureData(
      pictureName,
      L2DPLE_FirstPictureNumber + i * 6
    );
    this._weapon[i] = new PictureData(
      "",
      L2DPLE_FirstPictureNumber + 1 + i * 6
    );
    this._shield[i] = new PictureData(
      "",
      L2DPLE_FirstPictureNumber + 2 + i * 6
    );
    ...
    this._ornament[i] = new PictureData(
      "",
      L2DPLE_FirstPictureNumber + 5 + i * 6
    );
  }
};

Live2DPictureLinkEquipment.prototype.setPicture = function (
  member_No,
  equip_slotNo,
  cloth_Name,
  noClothName
...
Live2DPictureLinkEquipment.prototype.showPicture = function (x, y, name) {
  for (var i = 0; i < $gameParty._actors.length; i++) {
    if ($gameParty.members()[i]._name == name) {
      ...
      $gameScreen.showPicture(
        this._ornament[i].number,
        this._ornament[i].name,
        1,
        x,
        y,
        100,
