      ...
      switch (type) {
        ...
        case "switch":
          ...
          if (name != "Lost_Sensor_Switch" && !value.match(/^([A-D]|\d+)$/i)) {
            throw new Error(
              "Plugin parameter value is not switch : " + name + " : " + value
            );
          }
          break;
        default:
          throw new Error(
            "[CheckParam] " + name + "のタイプが不正です: " + type
          );
          break;
      }
    }
  ...
  Game_CharacterBase.prototype.isMapPassableEx = function (x, y, d) {
    ...
    if ($gameMap.isPassable(x, y, d) && $gameMap.isPassable(x2, y2, d2)) {
      ...
      if (regionDecisions.length > 0 && passableFlag == true) {
        passableFlag = !regionDecisions.contains(
          "" + $gameMap.regionId(x2, y2)
        );
      }
    } else {
      passableFlag = false;
  ...
  Bitmap.prototype.fillViewRangeLine = function (color, character) {
    ...
    if (direction == DIR_UP) {
      if (coordinates && cnt == 1) {
        ...
        this.mkrSideDrawLine(context, cx, cy, [
          sideSensorR,
          1,
          1,
          sideSensorL,
