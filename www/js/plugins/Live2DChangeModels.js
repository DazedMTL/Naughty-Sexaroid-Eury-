...
const L2DCM_modeldata = JSON.parse(
  JSON.stringify(L2DCM["ReSettingModelData"], L2DINstringifyReplacer)
);
...
Game_Live2d.prototype.Resetting_ReplaceModelData = function (
  destinationName,
  sourceName
) {
  var sourceData = null;
  ...
  if (sourceData != null) {
    for (var i = 1; i <= Object.keys(this.resettng_Modelname).length; i++) {
      if (destinationName == this.resettng_Modelname[i]) {
        ...
        this.resettng_individual_upsidedown[i] =
          sourceData.individual_upsidedown;
...
Game_Live2d.prototype.Resetting_UpdateModelData = function () {
  ...
  L2DINmodels.forEach(function (data) {
    ...
    this.individual_upsidedown[data_no] =
      this.resettng_individual_upsidedown[data_no];
