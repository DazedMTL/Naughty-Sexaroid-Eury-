...
(function ($) {
  ...
  Data_BattlerCommand.prototype.isEnabled = function (user) {
    return (
      this._enabled &&
      (!this._enableCondition ||
        this.evalCondition(this._enableCondition, user))
    );
  };
  ...
  Data_BattlerCommand.prototype.isVisible = function (user) {
    return (
      this._visible &&
      (!this._visibleCondition ||
        this.evalCondition(this._visibleCondition, user))
    );
  };
  ...
  $.Regex = /<battle[-_ ]command:\s*(\w+?)(?:\s+(\w+))?\s*\/>/gim;
  $.ExtRegex = /<battle[-_ ]command>([\s\S]*?)<\/battle[-_ ]command>/gim;
  ...
  Game_Battler.prototype.setBattleCommandEnabled = function (
    bool,
    symbol,
    ext
  ) {
  ...
  Game_Battler.prototype.setBattleCommandVisible = function (
    bool,
    symbol,
    ext
  ) {
  ...
  Game_Actor.prototype.refreshBattleCommands = function () {
    this.initBattleCommands();
    this._battleCommands = this._battleCommands.concat(
      this._extraBattleCommands
    );
  };
  ...
  var TH_WindowActorCommand_makeCommandList =
  ...
  var TH_SceneBattle_createActorCommandWindow =
