...
(function () {
  ...
  // ピクチャイージング設定の設定
  Game_Screen.prototype.setPictureEasing = function (
    pictureId,
    motionType,
    easingType
  ) {
  ...
  // 染色開始
  var _Game_Picture_tint = Game_Picture.prototype.tint;
  ...
  Game_Picture.prototype.updateMove = function () {
    if (this._duration > 0) {
      var durRate =
        (this._orgDuration - this._duration + 1) / this._orgDuration;
      ...
      this._scaleX =
        this._orgScaleX + difSclX * this.easingFunc("zoom")(durRate);
      this._scaleY =
        this._orgScaleY + difSclY * this.easingFunc("zoom")(durRate);
      this._opacity =
        this._orgOpacity + difOpc * this.easingFunc("fade")(durRate);
      this._duration--;
    }
  };
  ...
  Game_Picture.prototype.updateTone = function () {
    if (this._toneDuration > 0) {
      ...
      for (var i = 0; i < this._tone.length; i++) {
        ...
        this._tone[i] =
          this._toneOrg[i] + dif * this.easingFunc("tint")(durRate);
      }
      this._toneDuration--;
    }
  };
