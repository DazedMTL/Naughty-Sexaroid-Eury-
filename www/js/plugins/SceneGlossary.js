...
(function () {
  ...
  Window_ScrollText.prototype.startMessage = function () {
    ...
    if (param.AutoAddition)
      $gameParty.gainGlossaryFromText(
        this.convertEscapeCharacters(this._text),
        false
      );
  };
  ...
  Window_Glossary.prototype.applyViewingSwitch = function () {
    var metaValue = this.getMetaContents(
      ["用語閲覧スイッチ", "ViewingSwitch"],
      0
    );
    if (metaValue) {
  ...
  Window_Glossary.prototype.isNeedPageNumber = function () {
    return (
      param.ShowPageNumber &&
      this._maxPages > 1 &&
      !this.getMetaContents(["NoPageNum", "ページ番号不要"])
    );
  };
