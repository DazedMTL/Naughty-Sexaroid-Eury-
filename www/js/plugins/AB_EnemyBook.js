...
(function () {
  ...
  var ShowCommandInBattle =
    parameters["ShowCommandInBattle"] == 1 ? true : false;
  var EnemyBookAllCommandName = parameters["EnemyBookAllCommandName"] || "図鑑";
  var ShowAllBookCommandInBattle =
    parameters["ShowAllBookCommandInBattle"] == 1 ? true : false;
  ...
  var HideUnknownStatusInSkill =
    parameters["HideUnknownStatusInSkill"] == 1 ? true : false;
  ...
  var ShowCommandInBattle =
    parameters["ShowCommandInBattle"] == 1 ? true : false;
  var ShowGeneralStatusInSkill =
    parameters["ShowGeneralStatusInSkill"] == 1 ? true : false;
  var AddEnemySkillMessage = String(parameters["AddEnemySkillMessage"] || "");
  var FailToAddEnemySkillMessage = String(
    parameters["FailToAddEnemySkillMessage"] || ""
  );
  var MissToAddEnemySkillMessage = String(
    parameters["MissToAddEnemySkillMessage"] || ""
  );
  var FailToCheckEnemySkillMessage = String(
    parameters["FailToCheckEnemySkillMessage"] || ""
  );
  ...
  var UseElementIconInPluginParameter =
    parameters["UseElementIconInPluginParameter"] == 1 ? true : false;

  var DispDefeatNumber =
    Number(parameters["DispDefeatNumber"]) == 1 ? true : false;
  var dispParameters = [];
  ...
  var NoEffectStateName = String(
    parameters["NoEffectStateName"] || "無効ステート"
  );
  ...
  Game_System.prototype.isShowBattleAllEnemyBook = function () {
    if (this._showAllBookCommandInBattle === undefined)
      this.initEnemyBookSettings();
  ...
  Game_System.prototype.isShowCurrentEnemysStatus = function () {
    if (this._showCurrentEnemyStatus === undefined)
      this.initEnemyBookSettings();
  ...
  Scene_Battle.prototype.createEnemyBookWindows = function () {
    ...
    this._enemyBookIndexWindow.setHandler(
      "cancel",
      this.endBattleEnemyBook.bind(this)
    );
  ...
  Window_EnemyBookPercent.prototype.initialize = function (
    x,
    y,
    width,
    height
  ...
  Window_EnemyBookPercent.prototype.setAchievement = function (
    max,
    achievement
  ) {
    this.max = max;
  ...
  Window_EnemyBookPercent.prototype.refresh = function () {
    ...
    this.drawText(
      Math.floor((this.achievement / this.max) * 100) + "%",
      w1,
      0,
      w1,
      "right"
    ...
    if (this._backgroundSprite == undefined) {
      if (BackgroundImage) {
        ...
        this._backgroundSprite.bitmap =
          ImageManager.loadParallax(BackgroundImage);
...
*/
  ...
  Window_EnemyBookStatus.prototype.drawAllContents = function (
    x,
    y,
    width,
    height
    ...
    var column1x = x;
    var column2x = width
      ? x + width / 2 + this.standardPadding() / 2
      : this.contentsWidth() / 2 + this.standardPadding() / 2;
    var columnWidth = width
      ? width / 2 - this.standardPadding()
      : this.contentsWidth() / 2 - this.standardPadding();
    var x = x || 0;
    var y = y || 0;
    ...
    if (
      !enemy ||
      (this.isAllEnemies && !$gameSystem.isInEnemyBook(enemy.enemy()))
    ) {
...
*/
    ...
    if (DispDefeatNumber) {
      ...
      this.drawText(
        $gameSystem.defeatNumber(enemy.enemyId()),
        x + w,
        y,
        w,
        "right"
...
*/
    ...
    if (x == column2x) y += lineHeight * 2 + this.textPadding();
    x = column1x;

    if (!isHideStatus && DispDescribe) {
      for (var i = 1; i <= DescribeLineNumber; i++) {
        this.drawTextEx(
          dataEnemy.meta["desc" + i],
          x,
          y + lineHeight * (i - 1)
        );
      }
  ...
  Window_EnemyBookStatus.prototype.drawResistElement = function (x, y, w) {
    ...
    y += this.lineHeight();
    if (
      $gameSystem.isInEnemyBook(enemy.enemy()) ||
      (this.isCheck && !HideUnknownStatusInSkill)
    ) {
  ...
  Window_EnemyBookStatus.prototype.drawResistStates = function (x, y, w) {
    ...
    for (var i = 1, l = $dataStates.length; i < l; i++) {
      ...
      if (
        (rate < 1 || enemy.isStateResist(i)) &&
        $dataStates[i].meta.book !== "no"
      ) {
  ...
  Window_EnemyBookStatus.prototype.isCurrentStatus = function (enemy) {
    ...
    return (
      !this.isAllEnemies &&
      ($gameSystem.isShowCurrentEnemysStatus() || this.isCheck) &&
      !ShowGeneralStatusInSkill
    );
  };
  ...
  Game_Action.prototype.checkEnemyStatus = function (target) {
    this.makeSuccess(target);
    if (
      !(target.enemy().meta.book == "no" && !target.enemy().meta.bookCanCheck)
    ) {
  ...
  Scene_Battle.prototype.createActorCommandWindow = function () {
    ...
    this._actorCommandWindow.setHandler(
      "ab_enemybook",
      AB_EnemyBook.onCommand.bind(this)
    );
  };

  var _Window_ActorCommand_processHandling =
  ...
  var _Scene_Battle_createSkillWindow =
  ...
  var _Window_BattleSkill_processHandling =
    Window_BattleSkill.prototype.processHandling;
  ...
  Scene_Battle.prototype.createItemWindow = function () {
    ...
    this._itemWindow.setHandler(
      "ab_enemybook",
      AB_EnemyBook.onCommand.bind(this)
    );
  };

  var _Window_BattleItem_processHandling =
    Window_BattleItem.prototype.processHandling;
  ...
  var _Window_PartyCommand_processHandling =
    Window_PartyCommand.prototype.processHandling;
