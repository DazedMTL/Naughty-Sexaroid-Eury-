...
(function () {
  ...
  var displayNamePlate =
    parameters["DisplayNamePlate"] == "true" ? true : false;
  var standPictureBrightnessChangeWait = Number(
    parameters["StandPictureBrightnessChangeWait"]
  );
  ...
  Scene_Base.prototype.initialize = function () {
    ...
    //現在話している人の名前
    if ($currectSpeechingActorName == null) {
      $currectSpeechingActorName = "";
    }
    if ($pastSpeechingActorName == null) {
  ...
  Game_Interpreter.prototype.setCurrentSpeechingActorNameFromPictureId =
    function (_picId) {
    ...
    for (_key in $currentPictures) {
      if ($currentPictures[_key] == $currectSpeechingActorName) {
        $gameScreen.tintPicture(
          Number(_key),
          [0, 0, 0, 0],
          standPictureBrightnessChangeWait
        );
      } else {
        $gameScreen.tintPicture(
          Number(_key),
    ...
    text = text.replace(
      ...
      function () {
        ...
        if (text.search(/\x1bN\<([0-9]\d*|0)\>/gi) == 0) {
          if (useNamePlate)
            $currectSpeechingActorName = $gameActors.actor(arguments[1]).name();
          return Yanfly.nameWindow.refresh(
            $gameActors.actor(arguments[1]).name(),
            1
          );
        } else {
          ...
          if (
            ...
            if (this._textState.text[this._textState.index] != "\x1b") {
              ...
              this.processEscapeCharacter(
                this.obtainEscapeCode(this._textState),
                this._textState
              );
            }
          }
  ...
  Game_Interpreter.prototype.pluginCommand = function (command, args) {
    ...
    if ((command || "").toUpperCase() === "STANDPICTUREMANAGER") {
      ...
      if (args[1].search(/^[0-9]+$/) == 0) {
        $currentPictures[args[0]] = $gameActors
          .actor(args[1].replace(/[^0-9]/g, ""))
          .name();
      } else {
