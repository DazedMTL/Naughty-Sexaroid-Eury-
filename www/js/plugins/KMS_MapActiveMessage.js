...
(function () {
  ...
  var Const = {
    ...
    regex: {
      activeMessage:
        /<(?:アクティブメッセージ|ActiveMessage)\s*[:\s]\s*([^>]+)>/i,
      displayRange:
        /<(?:アクティブメッセージ距離|ActiveMessageRange)\s*[:\s]\s*(\d+)>/i,
      displayDuration:
        /<(?:アクティブメッセージ表示時間|ActiveMessageDuration)\s*[:\s]\s*(\d+)>/i,
  ...
  Params.messageCountMax = Math.max(
    parseIntWithDefault(pluginParams["Max message count"], 8),
    1
  );
  Params.balloonOffsetY = parseIntWithDefault(
    pluginParams["Balloon offset Y"],
    20
  );
  Params.balloonMargin = parseIntWithDefault(
    pluginParams["Balloon margin"],
    -8
  );
  Params.defaultRange = Math.max(
    parseIntWithDefault(pluginParams["Default range"], 4),
    1
  );
  Params.displayDuration = Math.max(
    parseIntWithDefault(pluginParams["Display duration"], 300),
    1
  );
  Params.fadeFrameCount = Math.max(
    parseIntWithDefault(pluginParams["Fade frame count"], 8),
    1
  );
  Params.fontSize = Math.max(
    parseIntWithDefault(pluginParams["Font size"], 18),
    1
  );
   ...
   * アクティブメッセージの登録
   */
  Game_Temp.prototype.pushMapActiveMessages = function (
    targetEvent,
    messages,
    needsLoop,
    isForced
  ...
  Game_Player.prototype.findAvailableMapActiveMessageEvents = function () {
    ...
    var candidateEvents = $gameMap.events().filter(function (event) {
      return (
        event.hasMapActiveMessage() &&
        !event.isMapActiveMessageShown() &&
        this.calcDistanceForMapActiveMessage(event) <=
          event.getMapActiveMessageRange()
      );
    }, this);
  ...
  Game_Event.prototype.setupMapActiveMessage = function () {
    ...
    if (!isNullOrUndefined(rangeMatch)) {
      this._mapActiveMessageRange = Math.max(
        parseIntWithDefault(rangeMatch[1], Params.defaultRange),
        1
      );
    }
    ...
    commentUnits.forEach(function (unit) {
      // メッセージ定義を解析
      ...
      if (!isNullOrUndefined(durationMatch)) {
        var duration = parseIntWithDefault(
          durationMatch[1],
          Params.displayDuration
        );
        ...
        if (this._mapActiveMessageList.length > 0) {
          this._mapActiveMessageList[
            this._mapActiveMessageList.length - 1
          ].duration = duration;
        }
      }
    }, this);
  ...
  Sprite_MapActiveMessageBalloon.prototype = Object.create(
    Sprite_Base.prototype
  );
  Sprite_MapActiveMessageBalloon.prototype.constructor =
    Sprite_MapActiveMessageBalloon;
   ...
   * 吹き出しを描画
   */
  Sprite_MapActiveMessageBalloon.prototype.drawBalloon = function (
    x,
    y,
    width,
    height
   ...
   * 吹き出しの背景を描画
   */
  Sprite_MapActiveMessageBalloon.prototype.drawBalloonBackground = function (
    x,
    y,
    width,
    height
    ...
    this.bitmap.blt(
      ...
      x + frame.margin,
      y + frame.margin,
      width - frame.margin * 2,
      height - frame.margin * 2
    );
   ...
   * 吹き出しの枠を描画
   */
  Sprite_MapActiveMessageBalloon.prototype.drawBalloonFrame = function (
    x,
    y,
    width,
    height
    ...
    function drawFrameV(sx, dx, dy) {
      ...
      this.bitmap.blt(
        srcImage,
        sx,
        frame.size + frame.lineLength,
        frame.size,
        frame.size,
        dx,
        dy + height - frame.size,
        frame.size,
        frame.size
      );
    }
    ...
    drawFrameV.call(
      this,
      frame.srcSize + frame.size + frame.lineLength,
      x + width - frame.size,
      y
    );
  ...
  Sprite_Character.prototype.isSameEvent = function (event) {
    if (
      isNullOrUndefined(this._character) ||
  ...
  Window_MapActiveMessage.prototype.initialize = function () {
    ...
    if (isNullOrUndefined(Window_MapActiveMessage.tempMessageBuffer)) {
      Window_MapActiveMessage.tempMessageBuffer = new Bitmap(
        Graphics.boxWidth,
  ...
  Window_MapActiveMessage.prototype.isFadingIn = function () {
    return (
      this._duration >= this._initialDisplayDuration - Params.fadeFrameCount
    );
  };
  ...
  Window_MapActiveMessage.prototype.isSameEvent = function (event) {
    ...
    // ページ番号まで同じ場合に同一と見なす
    if (
      this._event.eventId() === event.eventId() &&
  ...
  Window_MapActiveMessage.prototype.isEventInScreen = function () {
    ...
    return (
      x >= 0 && y >= 0 && x <= Graphics.boxWidth && y <= Graphics.boxHeight
    );
  };
  ...
  Window_MapActiveMessage.prototype.display = function (message, sprite) {
    ...
    this.printDebug(
      "Display new message for event " + this._event.eventId() + "."
    );
  ...
  Window_MapActiveMessage.prototype.updatePosition = function () {
    ...
    var balloonOffsetY =
      Const.balloonVertexSize.height - Params.balloonOffsetY - margin;
    var heightOffset =
      this.height +
      (!isNullOrUndefined(this._characterSprite)
        ? this._characterSprite.patternHeight()
        : $gameMap.tileHeight());
  ...
  Window_MapActiveMessage.prototype.updateOpacity = function () {
    ...
    if (this.isFadingIn()) {
      // フェードイン
      opacity =
        (255 * (this._initialDisplayDuration - this._duration)) /
        Params.fadeFrameCount;
  ...
  Window_MapActiveMessage.prototype.checkPlayerDistance = function () {
    // 強制表示 or 消去中は距離判定しない
    if (this.isForced() || this._duration <= Params.fadeFrameCount) {
      return;
    }
  ...
  Scene_Map.prototype.updateActiveMessageDisplay = function () {
    ...
    // 表示が完了したものを削除
    this._activeMessageWindowLayer.children
  ...
  Scene_Map.prototype.displayActiveMessage = function () {
    ...
    while (!isNullOrUndefined(message)) {
      ...
      if (!this.isActiveMessageDisplayed(message.event)) {
        var character = this._spriteset.findCharacterSpriteByEvent(
          message.event
        );
  ...
  var _Scene_Map_snapForBattleBackground =
