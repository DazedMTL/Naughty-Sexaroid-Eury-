...
(function () {
  ...
  Window_Gauge.prototype.windowX = function () {
    ...
    if (
      !$gameMap.event(this._gaugeNum) ||
      $gameMap.event(this._gaugeNum)._erased
    ) {
      return;
    }
  ...
  Window_Gauge.prototype.updateVisibility = function () {
    ...
    if (
      chara.isHideGauge() ||
      chara.isTransparent() ||
      !$gameMap.getGaugeInfo(this._gaugeNum)
    ) {
      this.hide();
    } else if (
      chara.characterName() == "" &&
      (chara.tileId() == 0 || !Params.TileGaugeEnable[0])
    ) {
      this.hide();
  ...
  DataManager.saveGameWithoutRescue = function (savefileId) {
    const gaugeInfos = $gameMap.getGaugeInfos()
      ? [...$gameMap.getGaugeInfos()]
      : [];
    const gaugePush = $gameMap.getGaugePush()
      ? [...$gameMap.getGaugePush()]
      : [];
    $gameMap.initGauge();
