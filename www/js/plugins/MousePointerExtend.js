...
(function () {
  ...
  for (var i = 1; i < 6; i++) {
    paramPointerTypes[i] = getParamString([
      "PointerType" + i,
      "形状タイプ" + i,
    ]);
    paramPointerSwitches[i] = getParamNumber([
      "PointerSwitch" + i,
      "スイッチ" + i,
    ]);
  }
  ...
  var paramDefaultType = getParamString([
    "DefaultType",
    "デフォルト形状タイプ",
  ]);
  var paramErasePointer = getParamBoolean(["ErasePointer", "キー入力で消去"]);
  var paramPointerPath = getParamString([
    "PointerPath",
    "ポインタファイルパス",
  ]);
  ...
  Graphics.setPointerType = function (type) {
    ...
    Object.keys(paramCustomImages).forEach(
      function (index) {
  ...
  Graphics._setPointerTypeCustom = function (type, index) {
    if (type === "url" + index && paramCustomImages[index]) {
      this._PointerType =
        "url(img/" +
        (paramPointerPath || "pictures") +
        "/" +
        paramCustomImages[index] +
        ".png), default";
    }
  };

  Graphics.updateMousePointer = function () {
    document.body.style.cursor = this._hiddenPointer
      ? "none"
      : this._PointerType;
  };
})();
