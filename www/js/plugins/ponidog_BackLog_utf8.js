...
*/
  ...
  var nBackLogButtonDownBigId = Number(
    parameters["nBackLogButtonDownBig"] || 0
  );

  var bHideMessageCall_In_SwitchNum = Number(
    parameters["bHideMessageCall_In_SwitchNum"] || 0
  );
  var bHideMessageCall_Out_SwitchNum = Number(
    parameters["bHideMessageCall_Out_SwitchNum"] || 0
  );
  ...
  var bPoniBKlog_RClickHideON = eval(
    parameters["bPoniBKlog_RClickHideON"] || "false"
  );
  ...
  var bBackLogNameOn = eval(
    parameters["bBackLogNameOn_forYepMessageCore"] || "false"
  );
...
*/
    ...
    if (
      command === "poniBacklog_CharaCommonOFF" ||
      command === "poni_18" ||
      command === "poni_17off"
    ) {
  ...
  Game_Message.prototype.add = function (text) {
    ...
    if (nNameSerchSwitch >= 0) {
      ...
      if (
        _switchNumActorCommon > 0 &&
        $gameSwitches.value(nAnimeOnSwitchNumId) == true
      )
        $gameSwitches.setValue(_switchNumActorCommon, true); //
    } //if(nNameSerchSwitch>=0){
  ...
  BackLog.prototype.BackLog_Call = function () {
    ...
    if (sBackLog.length > 0) {
      ...
      var nPageNow =
        parseInt(sBackLog.length / nBackLogLineNum) - nBackLogPage + 2;
  ...
  Window_Message.prototype.updateWait = function () {
    ...
    if (oBackLog._BackLogBusy) {
      ...
      if (TouchInput.isTriggered() || TouchInput.isCancelled()) {
        ...
        if (bHideMessageNow == 1)
          $gameSwitches.setValue(bHideMessageCall_Out_SwitchNum, true),
            (bHideMessageNow = 0); //ver1.05
      } //if (TouchInput.isTriggered () ||TouchInput.isCancelled() ){
    } // if (oBackLog._BackLogBusy)
...
if (this.isAnySubWindowActive()) {
    ...
    */
    ...
    if (oBackLog._BackLogBusy) {
      ...
      if (
        this.isTriggered() ||
        (TouchInput.isCancelled() && oBackLog.IsSmahoRClick())
      ) {
        if (oBackLog._BackLogBusy && oBackLog.ButtonPressUpDownRect("Upkey"))
          return true;
        if (oBackLog._BackLogBusy && oBackLog.ButtonPressUpDownRect("Downkey"))
          return true;
...
*/
      ...
      if (this._parentWindow.isOpening()) {
        ...
        for (var i = 1; i <= nBoxlength; i++) {
          var sNameBox = SceneManager._scene.children[i - 1]._lastNameText;
          if ((SceneManager._scene.children[i - 1]._lastNameText = ""))
...
*/
      ...
      if (this._parentWindow.visible == false) {
        this.visible = false;
      } else {
        this.visible = true;
      }
  ...
  Window_Message.prototype.nameboxHideBackLogScript = function () {
    ...
    for (var i = 1; i <= nBoxlength; i++) {
      if (this._nameWindow.parent.children[i - 1] != null) {
        //fix ver1.03a  [i]になってたので[i-1]へ。右クリックでネームボックスが残るバグはこれ
